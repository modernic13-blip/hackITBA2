      ...
      <div className="w-full max-w-4xl space-y-16">
        ...
        >
          <h2 className="text-3xl sm:text-4xl font-semibold text-foreground">
            Si hubieras empezado en 2021…
          </h2>
          <p className="mt-3 text-muted-foreground text-sm">
            Rendimiento simulado basado en tu perfil.
          </p>
        </motion.div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={animatedData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number) => [`$${value.toLocaleString()}`, "Valor del Portafolio"]}
                />
        ...
        {showMetrics && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="text-center p-6 rounded-xl border border-border bg-card">
              <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">Retorno Total</p>
              <p className="text-3xl font-semibold text-success">+{totalReturn}%</p>
            </div>
            <div className="text-center p-6 rounded-xl border border-border bg-card">
              <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">Peor caída temporal</p>
              <p className="text-3xl font-semibold text-foreground">{backtestMetrics.maxDrawdown}%</p>
            </div>
            <div className="text-center p-6 rounded-xl border border-border bg-card">
              <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">Consistencia de retornos</p>
        ...
        {showMetrics && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="text-center space-y-1">
            <p className="text-sm text-muted-foreground">No es perfecto. Pero sí adaptativo.</p>
            <p className="text-xs text-muted-foreground/70">Diseñado para responder, no predecir.</p>
          </motion.div>
        )}
      </div>
    </section>
